...
def ensure_functions_loaded():
    """Ensure external functions are loaded (lazy initialization)."""
    global _function_data, DefinedToolsArray, DefinedFunctions
    
    if _function_data is None:
        # Load external functions on first use
        _function_data = load_external_functions()
        
        # Use external function definitions
        DefinedToolsArray = _function_data["tools_array"]
